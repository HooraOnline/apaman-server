...
router.post('/backFromBank/apamanWeb', async function (req, res) {
    let bankData=await updatePaymentBeforReturnToApplication(req, res);
    logger.info('API: Payment/result Post44444555555555555555555 %j', {body: bankData});
...
router.post('/backFromBank/nasimWeb', async function (req, res) {
    let bankData=await updatePaymentBeforReturnToApplication(req, res);


    if(bankData.status==="0"){
        res.render("nasimWeb/bankPaymentSucsess", {
            payResult:bankData ,
        });
    }else if(bankData.status==="-138"){
        res.render("nasimWeb/bankPaymentCancel", {
            payResult:bankData ,
        });
    }else{
        res.render("nasimWeb/bankPaymentError", {
            payResult:bankData ,
        });
    }

});
router.post('/backFromBank/armaniWeb', async function (req, res) {
    let bankData=await updatePaymentBeforReturnToApplication(req, res);
    if(bankData.status==="0"  ){
        res.render("armaniWeb/bankPaymentSucsess", {
            payResult:bankData ,
        });
    }else if(bankData.status==="-138"){
        res.render("armaniWeb/bankPaymentCancel", {
            payResult:bankData ,
        });
    }else{
        res.render("armaniWeb/bankPaymentError", {
            payResult:bankData ,
        });
    }
});
router.post('/backFromBank/tazhWeb', async function (req, res) {
    let bankData=await updatePaymentBeforReturnToApplication(req, res);
    if(bankData.status==="0"  ){
        res.render("tazhWeb/bankPaymentSucsess", {
            payResult:bankData ,
        });
    }else if(bankData.status==="-138"){
        res.render("tazhWeb/bankPaymentCancel", {
            payResult:bankData ,
        });
    }else{
        res.render("tazhWeb/bankPaymentError", {
            payResult:bankData ,
        });
    }
});
...
function gatewayMultiplexTokenPayment(updatePaymentData, params, gatewayType, callBack) {
    ...
    let bankCallBackUrl;
   /* if(params.appKey.search('http')>-1){

        //bankCallBackUrl='http://apamanapp.ir/PayAnnounce';
        bankCallBackUrl='https://www.monta.ir/apaman/apaman198/pay/backFromBank/apamanWeb'//?site='+params.appKey;
    }*/
    const pecPayment = paymentFactory.getPecPayment();
