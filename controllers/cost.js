...
router.post('/addResult2/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:Offset.:Length', function (req, res) {
    logger.info('API: cost/addResult %j', {body: req.body, userId: req.userId});
    //connect to your database
    const shema = "Accounting";
    const sp = "Calculation";
    ...
    request.input('Length', sql.BigInt, req.params.Length || 20);
    request.output('CostCalcHeaderHistoryID', sql.BigInt);
    setSqlPublicParam(request,req.params,req.userId, sql.BigInt);
    request.execute(`${shema}.${sp}`, function (err, recordset) {
        if (err) logger.error("API: cost/addResult Error: %s", err);
...
router.get('/edit2/:CalculationHeaderID.:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:Offset.:Length', function (req, res) {
    ...
    //sp
    const shema = "Accounting";
    const sp = "Calculation_SelectWithCalculationHeaderIDNew";
