...
router.get('/search/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:Offset.:Length.:SearchValue.:JustPrev', function (req, res) {

    logger.info('API: Unit/Unit_Select_All_ByPaging %j', {params: req.params, token_userId: req.userId});
    ...
    request.input('JustPrev', sql.BIT, req.params.JustPrev=="true"?1: 0);
    if(req.params.SearchValue && req.params.SearchValue!=='null'){
        request.input('SearchValue', sql.NVARCHAR(200), req.params.SearchValue);
    }

    request.output('UnitCount', sql.BigInt);
    request.output('SumPrice', sql.BigInt);
    request.output('UnitAreas', sql.VARCHAR(1000));
    request.execute(`${shema}.${sp}`, function (err, recordset) {
        if (err) console.log(err);
        let result = null;
        if (recordset) {
        ...
        logger.info('API: Unit/Unit_Select_All Result222 %j', {code: 200, Response: result});
        // send data as a response
        // res.status(200).send(result);
        logger.info('API: Unit/Unit_Select_All returnvalue5555555555555 %j', recordset.output);
        CheckException.handler(res, result,recordset.output);
    });
});
...
router.get('/resident2/:UnitID.:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:CallerUserID', function (req, res) {
    ...
    // create Request object
    const request = new sql.Request(pool);
    if(req.params.UnitID==="null"){
        request.input('UnitID', sql.BigInt, null);
    }else{
        request.input('UnitID', sql.BigInt, req.params.UnitID);
    }
    setSqlPublicParam(request,req.params,req.userId, sql.BigInt);
...
router.post('/insertUser/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:CallerUserID',async function (req, res) {

    logger.info('API: Unit/UserInsertOrUpdateAdmin  %j', {params: req.params, body: req.body, userId: req.userId});

    //connect to your database
    //sp
    const shema = "Security";
    const sp = "UserUnit_InsertOrUpdateAdmin";
    ...
    //fields
    if (req.body.UserUnitID){
        request.input("UserUnitID",sql.BigInt,req.body.UserUnitID)
    }
    ...
    if (req.body.Password!==null){
        const password =await crypto.createHash('sha256').update(req.body.Password).digest('hex');
        let passBinBuff =await Buffer.from(password, 'hex');
        request.input('Password', sql.Binary(32),passBinBuff);
    }else{
        request.input('Password', sql.Binary(32),null);
    }
    request.input('Sex', sql.Bit, req.body.Sex);
    request.input('RoleID', sql.BigInt, req.body.RoleID);
    setSqlPublicParam(request,req.params,req.userId, sql.BigInt);
    request.execute(`${shema}.${sp}`, function (err, recordset) {
        if (err) logger.error("API: API: Unit/UserInsertOrUpdateAdmin Error: %s", err);

        let result = null;
        if (recordset) {
            result = recordset.recordset;
        }
        logger.info('API: Unit/UserInsertOrUpdateAdmin Result: %j', {code: 200, Response: result});
        // send data as a response
        // res.status(200).send(result);
        CheckException.handler(res, result);
