...
router.post('/sendPush', function (req, res) {
        logger.info('API: user/sendPush %j', {body: req.body, userId: req.userId});
    PushNotification.getSend();
        PushNotification.send(req.body.title, req.body.text, req.body.pushToken)
            .then(result => {
...
//______________________Search User________________//
router.get('/searchUser/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:Mobile.:Offset.:Length.:SearchValue.:JustPrev.:JustDebt', function (req, res) {
    logger.info('API: User/searchUser %j', {params: req.params, userId: req.userId});
    //connect to your database
    //sp
    const shema = "Security";
    const sp = "User_Search";
    ...
    request.input('JustDebt', sql.BIT, req.params.JustDebt=="true"?1: 0);
...
router.get('/role2/:FormID.:BuildingID', function (req, res) {
    ...
    if(req.params.BuildingID && req.params.BuildingID!='null'){
...
router.get('/firstUserBalanceNew2/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:Offset.:Length.:SearchValue', function (req, res) {
    logger.info('API: First_User_Balance2/Select %j', {params: req.params, userId: req.userId});
    //connect to your database
    //sp
    const shema = "Accounting";
    const sp = "FirstUserBalance_SelectNew";
    ...
    request.execute(`${shema}.${sp}`, function (err, recordset) {
        if (err) logger.error("API: First_User_Balance2/Select Error: %s", err);

        let result = null;
        if (recordset) {
            result = recordset.recordset;
        }
        logger.info('API: First_User_Balance2/Select Result: %j', {code: 200, Response: result});
        // send data as a response
...
router.get('/search2/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:CallerUserID.:Mobile', function (req, res) {
    ...
    if (req.params.CallerBuildingID==="null"){
        request.input('CallerBuildingID',sql.BigInt,null)
    }else{
        request.input('CallerBuildingID',sql.BigInt,null)
    }
    if (req.params.CallerUnitID==="null"){
        request.input('CallerUnitID',sql.BigInt,null)
    }else{
        request.input('CallerUnitID',sql.BigInt,null)
    }
