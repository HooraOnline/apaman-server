...
router.post('/defaultCharge/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID', function (req, res)  {
    ...
    if(req.body.DataArea){
        request.input('DataArea', sql.VarChar(1000), JSON.stringify(req.body.DataArea));
    }
    if(req.body.DataMeter){
        request.input('DataMeter', sql.BIGINT, req.body.DataMeter);
    }
    setSqlPublicParam(request,req.params,req.userId, sql.BigInt);
