...
router.get('/select/:ID/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:Offset.:Length.:SearchValue', function (req, res) {
    ...
    request.output('Mandeh', sql.BigInt);
...
router.get('/:ID/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID', function (req, res) {
    ...
    logger.info('API: Announcement/SelectAnnouncementWithId2222 %j', {params: req.params, userId: req.userId});
    ...
    request.input('ID', sql.BigInt, req.params.ID);
    setSqlPublicParam(request,req.params,req.userId, sql.BigInt);
    logger.info('API: Announcement/SelectAnnouncementWithId33333 %j', {params: req.params, userId: req.userId});
    request.execute(`${shema}.${sp}`, function (err, recordset) {
        if (err) logger.error("API: Announcement/SelectAnnouncementWithId Error: %s", err);
...
router.get('/forAnnouncement/defaultCharge2/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:PeriodDetailID.:Offset.:Length.:SearchValue', function (req, res) {

    logger.info('API: Announcement/SelectForAnnouncement_DefaultCharge2 %j', {params: req.params, userId: req.userId});
    ...
    request.output('SumPrice', sql.BigInt);
    request.execute(`${shema}.${sp}`, function (err, recordset) {
        if (err) logger.error("API: Announcement/SelectForAnnouncement_DefaultCharge2 Error: %s", err);

        let result = null;
        if (recordset) {
            result = recordset.recordset;
        }
        logger.info('API: Announcement/SelectForAnnouncement_DefaultCharge2 Result: %j', {code: 200, Response: result});
        // send data as a response
        CheckException.handler(res, result,recordset.output);
    });
});
...
router.get('/forAnnouncement/calculationHeader2/:CallerFormID.:CallerBuildingID.:CallerUnitID.:CallerRoleID.:CostClassID.:PeriodDetailID.:CalculationHeaderIDs.:Offset.:Length.:SearchValue', function (req, res) {
