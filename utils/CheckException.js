...
module.exports = {
    handler: (res, result,output={}) => {
        logger.info('output888888888888888888888 %j', output);
        ...
        try {
            if (!result || result.length === 0) {
                ...
                res.set({errCode: 0, errMessage: toUnicode("لیست خالی"),output:JSON.stringify(output )}).status(200).send([])
                ...
                res.set({errCode: 1001, errMessage: toUnicode("!DB ERR:" + result[0].dbErr),output:JSON.stringify(output )}).status(507).send(result);
                ...
                logger.info('*** ResultHandler result ok444444444 *** : ', result[0]);

                res.set({errCode: 1000, errMessage: toUnicode('دریافت اطلاعات'), output:JSON.stringify(output ) }).status(200).send(result);
            }
        } catch (e) {
